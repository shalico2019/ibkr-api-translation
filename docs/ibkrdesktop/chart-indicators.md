...
| **Williams %R** | 0 到 -100 区间的**超买/超卖**指标；%R = (HIGH(i-n) − Close) / (HIGH(i-n) − LOW(i-n)) × 100。 |
...
| **Keltner Channel（肯特纳通道）** | 基于 ATR 的中线 + 上下包络：价格穿越边界常提示**交易机会**。 |
...
- **公式来源**：上述公式与说明均来自 IBKR Desktop 官方 User Guide 字典（2025-10-07 版本），逐字逐项对应英文原文。
...
- 源站是一张**长字典表**（约 100 个指标），本翻译按**趋势 / 趋势强度 / 动量 / 波动率 / 成交量 / 通道 / 相关性 / 累计**八类重新组织，方便检索；**每个指标的描述 1:1 对应原文**，未删节亦未自创内容。
